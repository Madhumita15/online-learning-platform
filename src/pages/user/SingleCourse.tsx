...
import {
  ...
  deleteWishlist,
  getWishlist,
} from "../../stores/slices/wishlist.slice";
import Lottie from "lottie-react";
import { Box } from "@mui/material";
import loadingAnimation from "../../services/json/lottie/Loading animation.json";
import { getSingleCourse } from "../../stores/slices/course.slice";
import { toast } from "sonner";

const SingleCourse = () => {
  const { user } = useAppSeletor((state) => state.auth);
  const { singleIdCourse, loading } = useAppSeletor((state) => state.course);
  const { id } = useParams();
  const shorDesc = singleIdCourse?.description?.slice(0, 20);
  const { isEnrolledMap } = useAppSeletor((state) => state.enrollment);
  const { isWishlistedMap, allWishList } = useAppSeletor(
    (state) => state.wishlist,
  );

  const dispatch = useAppDispatch();

  const isEnrolled = singleIdCourse
    ? isEnrolledMap[singleIdCourse?.$id] === true
    : false;
  const isLoggedIn = !!user.userId;
  const isWishlisted = singleIdCourse
    ? isWishlistedMap[singleIdCourse?.$id] === true
    : false;

  // console.log("userId", user);
  const toggleWishlist = async (userId: string, courseId: string) => {
    const existingWishlist = allWishList.find(
      (wish) => wish.courseId === courseId && wish.userId === userId,
    );
    if (existingWishlist) {
      await dispatch(deleteWishlist({ id: existingWishlist.$id })).unwrap();
    } else {
      await dispatch(
        createWishList({ userId: userId, courseId: courseId }),
      ).unwrap();
    }
  };
  ...
  useEffect(() => {
    if (!user.userId || !singleIdCourse?.$id) return;
    dispatch(
      checkEnrollment({ courseId: singleIdCourse.$id, userId: user.userId }),
    );
  }, [dispatch, singleIdCourse?.$id, user.userId]);

  useEffect(() => {
    if (id) {
      dispatch(getSingleCourse({ rowId: id }));
    }
  }, [dispatch, id]);
  ...
  if (!singleIdCourse) return <h1>Course not found</h1>;
          ...
          <div className="flex flex-wrap items-center justify-center   gap-8 mb-8">
            <div className="flex items-center justify-center">
              <div className="relative group">
                ...
                <img
                  src={singleIdCourse.image}
                  alt={singleIdCourse.title}
            ...
            <div className="flex flex-col justify-center   space-y-7">
              ...
              <div className="flex flex-row gap-1">
                <button
                  ...
                  onClick={() =>
                    singleIdCourse && handleEnrollment(singleIdCourse.$id)
                  }
                ...
                >
                  ...
                  <span className="relative">
                    {isEnrolled && isLoggedIn ? "Enrolled" : "Enroll Now"}
                  </span>
                </button>
                ...
                >
                  ...
                  <Heart
                    className={`mr-2 h-5 w-5 transition-all duration-300 ${
                      isWishlisted && isLoggedIn ? "fill-current" : ""
                    }`}
                  />
                  <span className="relative">
                    {isWishlisted && isLoggedIn
